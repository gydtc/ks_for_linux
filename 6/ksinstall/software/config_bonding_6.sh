...
{ 
        ...
        sed -i s/^HOSTNAME.*/HOSTNAME=$(hostname)/ $GNWFILEPATH
...
configure_bonding()
{
        ...
        then
                ...
                set_rhel6_bond_config -b $BONDNAME -m 1 -i $IPADDR -n $NETMASK -g $GATEWAY -t static -s "miimon=100 primary=eth0"
                set_rhel6_ethx_config $BONDNAME $INTERFACE1
                set_rhel6_ethx_config $BONDNAME $INTERFACE2

                echo -e "alias $BONDNAME bonding" > /etc/modprobe.d/bonding.conf
...
bond_config_file="$NWIFILEPATH/ifcfg-$bond_name"
...
testing_network()
{
	...
	ipaddr=`ip route |grep default |awk '{ print $3 }'`
        if [ -n "$ipaddr" ]; then
             ...
             echo -e "[31mPlease configure gateway!!![m"
        fi
        echo 
}
...
configure_network()
{
        ...
        do
                ...
                echo -e "\t\t3. Clear network configure"
                echo -e "\t\t4. Restart network service"
